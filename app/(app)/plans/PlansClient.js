...
export default function PlansClient({ initialPlans = [] }) {
  ...
  const COUNTRY_BY_CURRENCY = Object.fromEntries(
    PUBLIC_DISPLAY.map((p) => [p.currency, p.country])
  );
  ...
  useEffect(() => {
    ...
    listPlans()
      ...
      .catch(() => {
        if (!alive) return;
        setError("Error al cargar planes. Por favor intenta mas tarde.");
      })
      .finally(() => {
        if (!alive) return;
        setLoading(false);
    ...
    <main className="mx-auto max-w-screen-md p-6 space-y-6">
      ...
      {plans.length === 0 ? (
        <div className="text-center text-gray-600">
          No hay planes disponibles en este momento. Por favor vuelve mas tarde.
        </div>
      ) : (
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {plans.map((plan) => {
            ...
            const renderSingle =
              ...
              hasCountry &&
              selectedCurrency &&
              PUBLIC_CODES.has(selectedCurrency) &&
              priceMap[selectedCurrency] != null;

            // When not logged in, only show MX/EU/AR (country names)
            const visiblePublic = PUBLIC_DISPLAY.filter((p) => priceMap[p.currency] != null);
              ...
              >
                ...
                <div className="space-y-4">
                  ...
                  {renderSingle ? (
                    <div className="text-center">
                      ...
                      <div className="text-sm text-gray-500">
                        {plan.durationDays} dia{plan.durationDays !== 1 ? "s" : ""} de acceso
                      </div>
                    </div>
                  ) : (
                  ...
                  {plan.features?.length > 0 && (
                    <div>
                      <h3 className="font-medium text-sm mb-2">Caracteristicas incluidas:</h3>
