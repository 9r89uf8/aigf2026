...
export default function GirlsClient({ initialGirls = [] }) {
  const girlsQuery = useQuery(api.girls.listGirlsPublic);
  const girls = girlsQuery ?? initialGirls;
  ...
  useEffect(() => {
    async function fetchAvatarUrls() {
      ...
      const avatarKeys = girls.map((g) => g.avatarKey).filter(Boolean);
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-3xl font-bold text-gray-900 mb-4">
            Aun no hay companeras de IA
          </h1>
          <p className="text-gray-600">Vuelve pronto para nuevas companeras.</p>
        </div>
      </div>
    );
  }
